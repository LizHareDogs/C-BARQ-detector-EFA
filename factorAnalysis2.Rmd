...
### Maximum Average Partial Method (MAP)
...
We are most interested in MAP, which is recommended by @watkins20. 
VSS is questionable since we don't want to necessarily find the simplest structure.  

Settings:  
- n = 20 (number of factors, suggested more than the number hypothesized)  
- rotate = varimax
fm = minres for least squares  
- n.obs = 734


``` {r, MAR, results="asis", echo=FALSE}
vss1 <- vss(imputedCorr, n = 20, rotate = "varimax",
            fm = "minres", n.obs = 734)
### which.min retrurns the location in the vector of the minimum value.
```

The minimum value is at `r which.min(vss1$map)` (not shown in graph above).
